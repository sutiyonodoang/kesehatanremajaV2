                ...
                <div class="card-body">
                    @if($consultation->status === 'menunggu')
                        <div class="alert alert-info text-center">
                            <i class="fas fa-hourglass-half me-2"></i> Menunggu respon dari admin.
                        </div>
                    @elseif($consultation->status === 'selesai')
                        <p class="text-center text-muted">Konsultasi ini sudah selesai.</p>
                    @else
                        <form action="{{ route('user.consultations.response', $consultation) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="mb-3">
