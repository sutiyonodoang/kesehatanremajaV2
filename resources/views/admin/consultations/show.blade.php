    ...
    <div class="row mb-3">
        <div class="col-12 d-flex justify-content-between">
            ...
            @if($consultation->status !== 'selesai')
                <form action="{{ route('admin.consultations.finish', $consultation) }}" method="POST">
                    @csrf
                    @method('PATCH')
    ...
    <div class="row">
        ...
        <div class="col-lg-6 mb-4">
            ...
            <div class="card shadow rounded-4">
                ...
                <div class="card-body">
                        <form action="{{ route('admin.consultations.response', $consultation) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <div class="mb-3">
                                <label for="response" class="form-label">Respon *</label>
                                <textarea name="response" id="response" rows="4" class="form-control @error('response') is-invalid @enderror" {{ $consultation->status === 'selesai' ? 'disabled' : '' }} required>{{ old('response') }}</textarea>
                                @error('response')
                            ...
                            <div class="mb-3">
                                ...
                                <input type="file" name="attachments[]" id="attachments" class="form-control" {{ $consultation->status === 'selesai' ? 'disabled' : '' }} multiple>
                                <small class="text-muted">Maksimal 10MB per file</small>
                            </div>
                            <button type="submit" class="btn btn-primary w-100" {{ $consultation->status === 'selesai' ? 'disabled' : '' }}>
